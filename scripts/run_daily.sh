#!/bin/zsh
# 매일 08:30 KST: 최근 7일 신규 자료 수집(daily) + 새 출처 탐색(discovery).
...
"$PY" -m tracker collect --mode daily     >> "$LOG" 2>&1 || echo "daily collect 경고" >> "$LOG"
"$PY" -m tracker collect --mode discovery >> "$LOG" 2>&1 || echo "discovery 경고" >> "$LOG"
